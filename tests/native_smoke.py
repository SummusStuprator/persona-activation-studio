...
try:
    ...
    e.reset();e.evaluate(ids);logits=e.logits();out={'model':e.model['name'],'layers':e.layers,'dim':e.dim,'abi':e.abi,'finite':bool(np.isfinite(logits).all()),'plan':e.load_plan,'status':'passed'}
finally:e.close()
