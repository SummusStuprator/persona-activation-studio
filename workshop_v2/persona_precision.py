...
def rounding_metrics(table,controls,pre,post,enabled):
    ...
    return {'max_rounding_error':float(np.max(raw)),'max_excess_error':float(np.max(excess)),
            'storage':'bfloat16','check':'elementwise half-ULP plus float32 arithmetic tolerance'}
