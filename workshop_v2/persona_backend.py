...
class Engine:
    ...
    def open(self,model,context=2048,**kwargs):
        ...
        import psutil
        required=model['base_bytes']+model['adapter_bytes']+int(1.25*2**30)
        if psutil.virtual_memory().available<required:
            raise MemoryError(f'Persona CPU load needs approximately {required/2**30:.1f} GiB available RAM. Close another model or choose a smaller base. No other process was stopped.')
        ...
        files=[Path(__file__),Path(__file__).with_name('persona_store.py')]
        code=hashlib.sha256(b''.join(p.read_bytes() for p in files)).hexdigest()[:12]
        self.abi=f'peft-chat-v1-torch{torch.__version__}-tf{transformers.__version__}-peft{peft.__version__}-cpu-bf16-fp32linear-{code}'
        model.update(runtime_profile='persona_peft_cpu',backend='persona_peft',activation_dtype='bfloat16',
        ...
        return {'weights_frozen':True,'adapter_tensors':len(saved),'hooks':self.layers,'backend':self.abi}
    ...
    def _hook(self,k):
        def apply(module,args,output):
            ...
            self.pre[k]=before[0,-1].cpu().numpy().copy()
            self.mean[k]=before[0].mean(0).cpu().numpy().copy()
            changed=h
            ...
            self.post[k]=changed[0,-1].detach().float().cpu().numpy().copy()
    ...
    def evaluate(self,tokens,all_logits=False):
        ...
        with torch.inference_mode():
            result=self.network(torch.as_tensor(ids.copy()).unsqueeze(0),past_key_values=self.past,
                                use_cache=True,**kw)
    ...
    def capture(self,which=0):
        ...
        return np.stack([table[k] for k in range(self.layers)])
    ...
    def close(self):
        ...
        self.network=None;self.past=None;self.blocks=[];gc.collect()
